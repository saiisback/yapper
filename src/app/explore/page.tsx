...
import { List, Map as MapIcon, SlidersHorizontal, MapPin, Users, Package } from "lucide-react";
...
export default function ExplorePage() {
  ...
  useEffect(() => {
    ...
    fetchEntities();
  }, [selectedCategory, selectedType, sortBy, isSearching]);
  ...
  const filteredEntities =
    isSearching && selectedType !== "all"
      ? entities.filter((e) => e.type === selectedType)
      : entities;

  const mapPlaces = filteredEntities
  ...
  return (
    <div className="mx-auto max-w-7xl px-5 py-6 sm:px-6 lg:px-8">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-[#A0A0A0]">Discover and search places, creators & products</p>
      </div>

      {/* Search */}
      <div className="mb-5">
        <SearchBar
          placeholder="Search places, creators, reviews..."
          onSearch={handleSearch}
        />
      </div>

      {/* Type tabs (show when searching) */}
      {isSearching && (
        <div className="mb-5 flex gap-1 overflow-x-auto rounded-full bg-[#111111] p-1 scrollbar-none">
          {TYPE_TABS.map((tab) => (
            <button
              key={tab.id}
          ...
          {isSearching && (
            <div className="mb-5 flex items-center justify-between">
              <p className="text-sm text-[#A0A0A0]">
                {filteredEntities.length + reviews.length} result{filteredEntities.length + reviews.length !== 1 ? "s" : ""} for &quot;{searchQuery}&quot;
              </p>
              <button
                onClick={() => {
                  setIsSearching(false);
                ...
                className="text-sm font-medium text-warm-yellow hover:underline"
              >
                Clear search
              </button>
            </div>
            ...
            {filteredEntities.length > 0 ? (
              ...
              <div className="col-span-full rounded-3xl bg-[#222222] border border-dashed border-[#333333] p-12 text-center">
                <p className="text-lg text-[#A0A0A0]">
                  {isSearching ? `No places found for "${searchQuery}"` : "No places found."}
                </p>
                <p className="mt-1 text-sm text-[#A0A0A0]/60">
                  {isSearching ? "Try different keywords." : "Try adjusting your filters or add a new place."}
                </p>
              </div>
            )}
          </div>

          {/* Matching reviews (search mode) */}
          {isSearching && reviews.length > 0 && (
            <div className="mt-10">
